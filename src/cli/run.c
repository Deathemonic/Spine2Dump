...
static int command_scan(const char* input_dir) {
    AssetBundle bundle = {};
...
static int command_inspect(const char* input_dir) {
    ...
    int result = asset_bundle_load_primary_pair(input_dir, &bundle);
    if (result == 0) {
        result = spine_backend_inspect(bundle.skel_files.items[0], bundle.atlas_files.items[0]);
    }
...
static int command_expressions(const char* input_dir) {
    AssetBundle bundle = {};

    int result = asset_bundle_load_primary_pair(input_dir, &bundle);
    if (result == 0) {
        result = spine_backend_list_expressions(bundle.skel_files.items[0],
                                                bundle.atlas_files.items[0]);
    }
...
static int command_dump_expressions(const DumpExpressionsOptions* options) {
    AssetBundle bundle = {};

    int result = asset_bundle_load_primary_pair(options->input_dir, &bundle);
    if (result == 0) {
...
static int command_dump(const DumpOptions* options) {
    ...
    ZF_LOGI("  trim mode: %s", options->trim_mode == RENDER_TRIM_ANIMATION
                                   ? "animation"
...
int cli_run(int argc, char** argv) {
    ...
    const OneDirCommand one_dir_commands[] = {
        {.name = "scan",        .handler = command_scan       },
        {.name = "inspect",     .handler = command_inspect    },
        {.name = "expressions", .handler = command_expressions},
    };
    ...
    if (strcmp(command, "dump-expressions") == 0) {
        DumpExpressionsOptions options;
        CliParseResult parsed = cli_parse_dump_expressions_command(command_argc, command_argv,
                                                                   &options);
        return cli_exit_for_parse_result(parsed, parsed == CLI_PARSE_OK
                                                     ? command_dump_expressions(&options)
                                                     : 0);
    }
